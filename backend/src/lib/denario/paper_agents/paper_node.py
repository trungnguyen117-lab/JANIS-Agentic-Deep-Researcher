...
async def add_citations_async(state, text, section_name):
    ...
    if f_temp1.exists():
        ...
        # Pass LLM to process_tex_file_with_references for arXiv search
        func = partial(process_tex_file_with_references, text, state["keys"], None, state["llm"]["llm"])
...
async def citations_node(state: GraphState, config: RunnableConfig):
    ...
    # compile latex
    # Compile LaTeX - wrap in asyncio.to_thread to avoid blocking
    # Note: asyncio is already imported at the top of the file
    await asyncio.to_thread(compile_latex, state, state['files']['Paper_v3'])
    ...
    for section_name in sections:
        ...
        if f_temp.exists():
            ...
            # Wrap LLM_call in asyncio.to_thread to avoid blocking the event loop
            # LLM_call uses blocking invoke() which internally uses socket connections and time.sleep
            state, result = await asyncio.to_thread(LLM_call, PROMPT, state)
            section_text = extract_latex_block(state, result, "Text")
            # Wrap LaTeX_checker in asyncio.to_thread - it also calls LLM_call internally
            section_text = await asyncio.to_thread(LaTeX_checker, state, section_text)  #check LaTeX
    ...
    # Wrap compile_latex in asyncio.to_thread to avoid blocking the event loop
    await asyncio.to_thread(compile_latex, state, state['files']['Paper_v4'])
