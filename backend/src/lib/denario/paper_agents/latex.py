...
def compile_tex_document(state: dict, doc_name: str, doc_folder: str) -> None:
    """
    Compile a LaTeX document using xelatex and bibtex.
    
    NOTE: This function uses blocking subprocess.run() calls. It should only be
    called from sync contexts (not from async functions). If you need to call this
    from an async context, wrap it with asyncio.to_thread().
    """
...
def compile_latex(state: GraphState, paper_name: str) -> None:
    ...
    Note: This function contains blocking subprocess calls. When called from async
    contexts (like citations_node), it should be wrapped in asyncio.to_thread().
    """
    ...
    def run_xelatex():
        """Run xelatex compilation - blocking operation."""
    ...
    def run_bibtex():
        """Run bibtex compilation - blocking operation."""
