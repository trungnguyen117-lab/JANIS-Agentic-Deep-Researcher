...
def build_graph(mermaid_diagram: bool = False, outline: dict | None = None):
    """
    This function builds the graph.
    
    If an outline is provided, builds a dynamic graph based on the outline sections.
    Otherwise, builds the hardcoded 4-section graph for backward compatibility.
    
    Args:
        mermaid_diagram: Whether to generate a mermaid diagram
        outline: Optional outline dict with 'sections' key for dynamic graph building
    """
    # If outline is provided, use dynamic graph builder
    if outline is not None:
        try:
            from .dynamic_graph import build_dynamic_graph
            return build_dynamic_graph(outline=outline, mermaid_diagram=mermaid_diagram)
        except ImportError:
            # Fall back to hardcoded graph if dynamic_graph is not available
            pass
    
    # Default hardcoded graph (backward compatibility)
    # Define the graph
