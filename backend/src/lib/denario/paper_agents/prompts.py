...
def abstract_prompt(state, attempt):
    ...
    # Get outline guidance if available - check if there's a research topic/theme from outline
    outline_guidance = ""
    if 'outline' in state and state.get('outline') and isinstance(state['outline'], dict):
        sections = state['outline'].get('sections', [])
        if sections:
            # Use the introduction section description as guidance for the abstract
            intro_section = next((s for s in sections if s.get('id') == 'intro' or s.get('title', '').lower() == 'introduction'), None)
            if intro_section and intro_section.get('description'):
                outline_guidance = f"""

**IMPORTANT - Research Topic Guidance:**
The paper should focus on: {intro_section.get('description', '')}
"""
...
def introduction_prompt(state):
    ...
    # Get outline section description if available
    outline_guidance = ""
    if 'outline' in state and state.get('outline') and isinstance(state['outline'], dict):
        sections = state['outline'].get('sections', [])
        intro_section = next((s for s in sections if s.get('id') == 'intro' or s.get('title', '').lower() == 'introduction'), None)
...
**IMPORTANT - Follow this outline guidance for the Introduction section:**
- Section Title: {intro_section.get('title', 'Introduction')}
- Section Description: {intro_section.get('description', '')}
"""
            if intro_section.get('subsections'):
                outline_guidance += "\n- Subsections to include:\n"
                for subsec in sorted(intro_section.get('subsections', []), key=lambda x: x.get('order', 0)):
                    outline_guidance += f"  * {subsec.get('title', '')}: {subsec.get('description', '')}\n"
...
**IMPORTANT - Follow this outline guidance for the Methods section:**
- Section Title: {methods_section.get('title', 'Methods')}
- Section Description: {methods_section.get('description', '')}
"""
            if methods_section.get('subsections'):
                outline_guidance += "\n- Subsections to include:\n"
                for subsec in sorted(methods_section.get('subsections', []), key=lambda x: x.get('order', 0)):
                    outline_guidance += f"  * {subsec.get('title', '')}: {subsec.get('description', '')}\n"
...
**IMPORTANT - Follow this outline guidance for the Results section:**
- Section Title: {results_section.get('title', 'Results')}
- Section Description: {results_section.get('description', '')}
"""
            if results_section.get('subsections'):
                outline_guidance += "\n- Subsections to include:\n"
                for subsec in sorted(results_section.get('subsections', []), key=lambda x: x.get('order', 0)):
                    outline_guidance += f"  * {subsec.get('title', '')}: {subsec.get('description', '')}\n"
...
def conclusions_prompt(state):
    ...
    if 'outline' in state and state.get('outline') and isinstance(state['outline'], dict):
        sections = state['outline'].get('sections', [])
        conclusions_section = next((s for s in sections if s.get('id') == 'conclusions' or 'conclusion' in s.get('title', '').lower()), None)
        if conclusions_section:
            outline_guidance = f"""

**IMPORTANT - Follow this outline guidance for the Conclusions section:**
- Section Title: {conclusions_section.get('title', 'Conclusions')}
- Section Description: {conclusions_section.get('description', '')}
"""
            if conclusions_section.get('subsections'):
                outline_guidance += "\n- Subsections to include:\n"
                for subsec in sorted(conclusions_section.get('subsections', []), key=lambda x: x.get('order', 0)):
                    outline_guidance += f"  * {subsec.get('title', '')}: {subsec.get('description', '')}\n"
