...
def validate_json(json_string: Optional[str] = None, file_path: Optional[str] = None) -> str:
    ...
    - `file_path`: Path to a JSON file to read and validate (e.g., "project/plan_outline.json")
    ...
    Examples:
        ...
        validate_json(file_path="project/plan_outline.json")
    ...
    if file_path:
        # Read from file
        # Try multiple path resolutions: absolute, relative to cwd, and relative to project root
        ...
        if not path_obj.is_absolute():
            ...
            if cwd_path.exists():
                ...
                # Try relative to project root (assuming we're in backend/)
        ...
        try:
            with open(path_obj.resolve(), 'r', encoding='utf-8') as f:
                json_content = f.read()
            result_parts.append(f"📄 Reading JSON from file: {path_obj}")
            result_parts.append("")
        except FileNotFoundError:
            return f"❌ ERROR: File not found: {file_path} (tried: {path_obj})"
