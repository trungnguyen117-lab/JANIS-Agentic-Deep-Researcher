...
try:
    ...
    Langfuse = None  # Optional dependency
    CallbackHandler = None  # Optional dependency
from langchain_core.callbacks import BaseCallbackHandler
from langchain_core.outputs import LLMResult


class TokenTrackingCallbackHandler(CallbackHandler if CallbackHandler else BaseCallbackHandler):
    ...
    def __init__(self, public_key: Optional[str] = None, **kwargs):
        ...
        self._add_usage = None  # Disabled - deepagents removed
    
    def on_llm_end(self, response: LLMResult, **kwargs: Any) -> None:
        """Track token usage when LLM call ends."""
        # Call parent to send to Langfuse (if available)
        if CallbackHandler:
        ...
        if response.llm_output and "token_usage" in response.llm_output:
            ...
            # Token usage tracking disabled (deepagents removed)
            # if thread_id and self._add_usage:
            #     self._add_usage(...)
...
def get_langfuse_handler() -> Optional[Any]:
    ...
    """
    # If Langfuse is not installed, return None
